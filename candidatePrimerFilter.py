...
if not os.path.exists('candidateFilteredPrimers/'): #creates directory to store the files generated from the script
    os.makedirs('candidateFilteredPrimers/',exist_ok=True)

badDict = {} #dictionary to hold the orthogroup and associated bad primers as decided from specificity testing
with open('VennDiagram/sharedPrimerNamesFile.txt','r') as b: #file that contains the bad primer pair information
...
#Need redundant N filtering because both the initial filter for specificity pull from the same place, primer3 files and therefore there is no master currated list to pull from
                        if checkName not in badPrimerList: #logic to skip a bad primer pair
                            break
...
#This is for filtering primer pairs that have the same captured SNPs
    ...
    while olf < len(snpDict):
        ...
        for k1 in list(snpDict.keys()):
            ...
            keyTrac.append((k1,total0))

        olf += 1

    
    keyTrac.sort(key=lambda x:x[1])    
    ...
    lprimer = primerDict.get(bestDegen[0])[0]
    rprimer = primerDict.get(bestDegen[0])[1]
    primerGroup = bestDegen[0].strip('=')[9:]
    fileName = bestDegen[0][:9]
    primersOut = open('candidateFilteredPrimers/'+fileName+'filteredPrimers','a')
